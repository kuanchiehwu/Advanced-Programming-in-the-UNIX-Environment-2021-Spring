...
void _setjmp_sigmask(jmp_buf env){
	sigprocmask(SIG_BLOCK, NULL, &env->mask);
}
void _longjmp_sigmask(jmp_buf env){
	sigprocmask(SIG_UNBLOCK, &env->mask, NULL);
}
...
long sigaction(int how, const struct sigaction *nact, struct sigaction *oact){
	...
	if(nact){
		kact.k_sa_handler = nact->sa_handler;
		kact.sa_mask = nact->sa_mask;
		kact.sa_flags = nact->sa_flags | SA_RESTORER;
		kact.sa_restorer = &__myrt;
	}
	long ret = sys_rt_sigaction(how, nact? &kact : NULL, oact ? &koact : NULL, sizeof(sigset_t));
